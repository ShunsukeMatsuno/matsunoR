...
#' @param show_system Logical. If `TRUE`, print the system time row. Default is `FALSE`.
#'
...
#'       Rows are `cpu` and `elapsed` (and `system` when `show_system = TRUE`).}
#'     \item{speedup}{`user / elapsed` ratio (`Inf` if elapsed is zero)}
#'   }
#'
#' @details
...
exec_time <- function(expr, s_digits = 2, day_digits = 1, show_system = FALSE) {
  ...
  w_metric <- max(nchar(c("metric", df$name), type = "width"))
  ...
  invisible(list(
    times_sec = c(user = time_user, system = time_system, elapsed = time_elapsed),
    display   = df,
